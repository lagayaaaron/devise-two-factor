...
class User < ApplicationRecord
  ...
  # ============================================
  # TOTP Methods (Authenticator App)
  # ============================================
  ...
  def regenerate_backup_codes!
    plain_codes = Array.new(BACKUP_CODES_COUNT) do
      SecureRandom.hex(4) # 8 chars
    end

    self.backup_codes_digest =
      plain_codes.map { |code| BCrypt::Password.create(code) }
  ...
  # ============================================
  # Universal OTP Verification (Both Systems)
  # ============================================
  ...
  def verify_any_otp(code)
    ...
    case code.length
    when 6
      # Could be TOTP or Email OTP
      # Try TOTP first (if available), then email OTP
      if otp_secret.present?
        # Try both TOTP methods
